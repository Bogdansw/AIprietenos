const DATA = {

  heroStats: [
    { num: "37%", label: "Creștere anuală AI" },
    { num: "300M+", label: "Utilizatori AI global" },
    { num: "2025", label: "An de referință" },
  ],

  robotMessages: [
    "Bună ziua!", "Eu sunt AI", "Învăț din date",
    "Pot să ajut!", "Fii responsabil",
  ],

  robotBubbles: [
    { text: "Procesez date!", top: "10px", delay: "0s" },
    { text: "Idee nouă!", top: "70px", delay: "2s" },
    { text: "Gata!", top: "130px", delay: "4s" },
  ],

  gallery: [
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Educație", large: true },
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Medicină", large: false },
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Transport", large: false },
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Securitate", large: false },
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Divertisment", large: false },
    { img: "https://blocks.astratic.com/img/general-img-landscape.png", label: "AI în Cercetare", large: false },
  ],

  ceEste: {
    label: "01 — Definiție",
    title: "Ce este Inteligența Artificială?",
    paragraphs: [
      "<strong>Inteligența artificială (AI)</strong> este o tehnologie care permite calculatoarelor și programelor să analizeze informații, să învețe din date și să ofere răspunsuri sau predicții.",
      "În loc să execute doar instrucțiuni simple, sistemele AI pot identifica tipare, pot rezolva probleme și pot <strong>automatiza procese</strong> care înainte necesitau intervenția umană.",
      "Astăzi, AI este folosit pentru lucruri precum asistenți virtuali, traduceri automate, recomandări de conținut sau analiză de date. Scopul acestei tehnologii este de a <strong>sprijini oamenii</strong> și de a face procesele mai rapide și mai eficiente.",
    ],
    image: "https://blocks.astratic.com/img/general-img-landscape.png",
    imageAlt: "Ce este inteligența artificială",
    imageTint: "alt-image-tint-purple",
    imagePlaceholder: "Ilustrație definiție AI",
  },

  domenii: {
    label: "02 — Aplicații",
    title: "Unde este folosit AI",
    paragraphs: [
      "Inteligența artificială a pătruns în aproape toate domeniile activității umane. De la aplicații pe care le folosim zilnic până la sisteme complexe din industrie, AI ajută la <strong>automatizarea proceselor</strong> și la analiza rapidă a datelor.",
    ],
    image: "https://blocks.astratic.com/img/general-img-landscape.png",
    imageAlt: "Domenii de utilizare AI",
    imageTint: "alt-image-tint-green",
    imagePlaceholder: "Ilustrație domenii",
    cards: [
      { img: "images/icon-educatie.svg", title: "Educație", desc: "Platforme care personalizează procesul de învățare pentru fiecare elev." },
      { img: "images/icon-medicina.svg", title: "Medicină", desc: "Analiză de imagini medicale și diagnostic asistat de calculator." },
      { img: "images/icon-afaceri.svg", title: "Afaceri", desc: "Analiză de date și automatizarea proceselor repetitive." },
      { img: "images/icon-divertisment.svg", title: "Divertisment", desc: "Recomandări de filme, muzică sau conținut online personalizat." },
      { img: "images/icon-securitate.svg", title: "Securitate cibernetică", desc: "Detectarea amenințărilor și a atacurilor informatice în timp real." },
    ],
  },

  avantaje: {
    label: "03 — Beneficii",
    title: "Avantajele AI",
    paragraphs: [
      "Tehnologia AI aduce beneficii reale atunci când este utilizată corect. Aceasta poate <strong>accelera procesele</strong>, poate reduce erorile și poate ajuta la luarea unor decizii mai informate.",
      "Folosirea responsabilă a AI poate contribui la dezvoltarea tehnologică și la crearea unor soluții eficiente pentru probleme complexe.",
    ],
    list: [
      "Automatizarea sarcinilor repetitive",
      "Analiza rapidă a unor cantități mari de date",
      "Creșterea productivității în multe domenii",
      "Sprijin pentru cercetare și inovație",
      "Îmbunătățirea serviciilor digitale",
    ],
    image: "https://blocks.astratic.com/img/general-img-landscape.png",
    imageAlt: "Avantajele AI",
    imageTint: "alt-image-tint-orange",
    imagePlaceholder: "Ilustrație avantaje",
  },

  riscuri: {
    label: "04 — Atenție",
    title: "Riscuri și Provocări",
    paragraphs: [
      "O utilizare responsabilă a inteligenței artificiale presupune să înțelegem și riscurile pe care le poate implica. Deși AI poate fi foarte util, <strong>utilizarea necorespunzătoare</strong> poate duce la probleme serioase.",
      "De aceea, este important ca AI să fie utilizat responsabil și cu spirit critic.",
    ],
    list: [
      "Generarea de informații incorecte sau înșelătoare",
      "Probleme legate de confidențialitatea datelor",
      "Bias sau discriminare în algoritmi",
      "Utilizarea tehnologiei pentru manipulare sau dezinformare",
      "Impact asupra anumitor locuri de muncă",
    ],
    image: "https://blocks.astratic.com/img/general-img-landscape.png",
    imageAlt: "Riscuri AI",
    imageTint: "alt-image-tint-red",
    imagePlaceholder: "Ilustrație riscuri",
  },

  siguranta: {
    label: "05 — Bune practici",
    title: "Cum să folosești AI în siguranță",
    paragraphs: [
      "Pentru a folosi inteligența artificială în mod responsabil, este important să respectăm câteva reguli simple.",
      "Utilizată corect, AI poate fi un <strong>instrument extrem de util</strong> în învățare, muncă și creativitate.",
    ],
    list: [
      "Evită introducerea datelor personale sau sensibile",
      "Verifică informațiile generate de AI din mai multe surse",
      "Folosește AI ca instrument de sprijin, nu ca singura sursă de adevăr",
      "Analizează critic rezultatele oferite de sistemele AI",
      "Respectă regulile etice și legale atunci când utilizezi aceste tehnologii",
    ],
    image: "https://blocks.astratic.com/img/general-img-landscape.png",
    imageAlt: "Siguranță AI",
    imageTint: "alt-image-tint-blue",
    imagePlaceholder: "Ilustrație siguranță",
  },

  comparatie: {
    bine: {
      header: "Utilizare corectă",
      items: [
        "Generarea de idei sau brainstorming",
        "Rezumarea și organizarea informațiilor",
        "Traduceri și explicații pentru studiu",
        "Analiză de date sau sprijin în proiecte",
        "Scrierea unui draft pe care îl editezi tu",
        "Asistență în învățare — explicarea conceptelor",
      ],
    },
    rau: {
      header: "Utilizare incorectă",
      items: [
        "Răspândirea de informații neverificate",
        "Copierea conținutului fără înțelegere",
        "Folosirea AI pentru manipulare sau înșelăciune",
        "Decizii importante fără consultarea experților",
        "Introducerea datelor personale sensibile",
        "Acceptarea răspunsurilor fără verificare",
      ],
    },
  },

  quiz: [
    {
      q: "Ce este inteligența artificială?",
      opts: [
        "Un robot fizic care gândește ca omul",
        "O tehnologie care analizează date și generează răspunsuri",
        "Un program de jocuri video",
        "O rețea de calculatoare conectate la internet",
      ],
      correct: 1,
      feedback: "Corect! AI este o tehnologie care analizează informații, învață din date și oferă răspunsuri sau predicții.",
    },
    {
      q: "Ce NU ar trebui să introduci niciodată într-un chatbot AI?",
      opts: [
        "O întrebare despre matematică",
        "O cerere de traducere",
        "Parola ta sau CNP-ul",
        "O idee de brainstorming",
      ],
      correct: 2,
      feedback: "Exact! Niciodată nu introduce parole, CNP sau date financiare în platforme AI publice.",
    },
    {
      q: "Cum trebuie să tratezi informațiile oferite de AI?",
      opts: [
        "Le accept mereu ca adevăr absolut",
        "Le ignor complet",
        "Le verific din surse de încredere",
        "Le public imediat pe rețelele sociale",
      ],
      correct: 2,
      feedback: "Bravo! Informațiile AI trebuie mereu verificate din surse sigure înainte de a le folosi.",
    },
    {
      q: "Care este un avantaj real al inteligenței artificiale?",
      opts: [
        "Poate înlocui complet gândirea umană",
        "Analizează rapid cantități mari de date",
        "Are întotdeauna dreptate",
        "Nu face niciodată greșeli",
      ],
      correct: 1,
      feedback: "Corect! AI poate procesa volume uriașe de date mult mai rapid decât un om, dar poate și greși.",
    },
    {
      q: "AI poate genera informații greșite?",
      opts: [
        "Nu, AI este mereu precis",
        "Da, uneori produce erori sau informatii false",
        "Doar dacă este stricat",
        "Numai dacă îl întrebi greșit",
      ],
      correct: 1,
      feedback: "Exact! AI poate genera informații plauzibile dar false. De aceea verificarea surselor este esențială.",
    },
  ],

  faq: [
    {
      q: "Este inteligența artificială periculoasă?",
      a: "AI nu este în mod direct periculos, dar poate crea probleme dacă este folosit fără responsabilitate sau fără verificarea informațiilor. Ca orice instrument, pericolul vine din modul de utilizare, nu din tehnologia în sine.",
    },
    {
      q: "Poate AI să înlocuiască oamenii?",
      a: "AI poate automatiza anumite sarcini, dar în majoritatea cazurilor funcționează ca un instrument care sprijină activitatea umană. Nu poate înlocui creativitatea, empatia și judecata etică specifice omului.",
    },
    {
      q: "Sunt datele mele în siguranță când folosesc AI?",
      a: "Depinde de modul în care sunt folosite platformele AI. Este recomandat să nu introduci date personale sau informații sensibile. Citește mereu politica de confidențialitate înainte de a folosi un serviciu nou.",
    },
    {
      q: "Cât de corecte sunt răspunsurile AI?",
      a: "Deși AI poate oferi informații utile, uneori poate genera erori sau răspunsuri incorecte. De aceea, este important să verifici informațiile din surse sigure, mai ales pentru subiecte importante.",
    },
  ],

  footer: {
    desc: "Un proiect educativ despre utilizarea responsabilă a inteligenței artificiale. Informare clară pentru toată lumea.",
    cols: [
      {
        title: "Secțiuni",
        links: [
          { label: "Ce este AI", href: "index.html#ce-este" },
          { label: "Domenii", href: "index.html#domenii" },
          { label: "Avantaje", href: "index.html#avantaje" },
          { label: "Riscuri", href: "index.html#riscuri" },
          { label: "Siguranță", href: "index.html#siguranta" },
        ],
      },
      {
        title: "Resurse externe",
        links: [
          { label: "UNESCO AI Ethics", href: "https://www.unesco.org/en/artificial-intelligence" },
          { label: "EU AI Act", href: "https://artificialintelligenceact.eu/" },
          { label: "Partnership on AI", href: "https://partnershiponai.org/" },
        ],
      },
    ],
  },
};

function imgOrPh(src, alt, phText, extraClass) {
  return `
    <div class="alt-image ${extraClass || ''}">
      <img src="${src}" alt="${alt}" />
    </div>
  `;
}

function renderHeroStats() {
  document.getElementById("hero-stats").innerHTML = DATA.heroStats
    .map(s => `<div class="stat-item"><span class="stat-num">${s.num}</span><span class="stat-label">${s.label}</span></div>`)
    .join("");
}

function renderFloatingShapes() {
  const colors = ["#ede6ff","#deeeff","#dff5e8","#ffe8dc","#fff5d6","#fde8f0"];
  const wrap = document.getElementById("floating-shapes");
  if (!wrap) return;
  for (let i = 0; i < 12; i++) {
    const size = 40 + Math.random() * 80;
    const el = document.createElement("div");
    el.className = "fshape";
    el.style.cssText = `width:${size}px;height:${size}px;background:${colors[Math.floor(Math.random()*colors.length)]};left:${Math.random()*100}%;top:${Math.random()*100}%;animation-duration:${6+Math.random()*10}s;animation-delay:${-Math.random()*10}s;`;
    wrap.appendChild(el);
  }
}

function renderRobotBubbles() {
  const wrap = document.getElementById("robot-bubbles");
  if (!wrap) return;
  wrap.innerHTML = DATA.robotBubbles.map(b =>
    `<div class="bubble" style="top:${b.top};animation-delay:${b.delay}">${b.text}</div>`
  ).join("");
  let idx = 0;
  const textEl = document.getElementById("robot-text");
  if (!textEl) return;
  setInterval(() => {
    idx = (idx + 1) % DATA.robotMessages.length;
    textEl.textContent = DATA.robotMessages[idx];
  }, 3000);
}

function renderGallery() {
  const el = document.getElementById("photo-gallery");
  if (!el) return;
  el.innerHTML = DATA.gallery.map(item => `
    <div class="pg-item ${item.large ? 'large' : ''} reveal">
      <img src="${item.img}" alt="${item.label}" />
      <div class="pg-label">${item.label}</div>
    </div>
  `).join("");
}

function renderCeEste() {
  const el = document.getElementById("ce-este-content");
  if (!el) return;
  const d = DATA.ceEste;
  el.innerHTML = `
    <div class="alt-section reveal">
      <div class="alt-text">
        <div class="section-label">${d.label}</div>
        <h2 class="section-title">${d.title}</h2>
        ${d.paragraphs.map(p => `<p>${p}</p>`).join("")}
      </div>
      ${imgOrPh(d.image, d.imageAlt, d.imagePlaceholder, d.imageTint)}
    </div>
  `;
}

function renderDomenii() {
  const el = document.getElementById("domenii-content");
  if (!el) return;
  const d = DATA.domenii;
  el.innerHTML = `
    <div class="alt-section reverse reveal">
      ${imgOrPh(d.image, d.imageAlt, d.imagePlaceholder, d.imageTint)}
      <div class="alt-text">
        <div class="section-label">${d.label}</div>
        <h2 class="section-title">${d.title}</h2>
        ${d.paragraphs.map(p => `<p>${p}</p>`).join("")}
        <div class="domain-cards-inline">
          ${d.cards.map(c => `
            <div class="domain-card">
              <div class="domain-icon">
                <img src="${c.img}" alt="${c.title}" onerror="this.style.display='none'" />
                <div class="icon-ph"></div>
              </div>
              <h3>${c.title}</h3>
              <p>${c.desc}</p>
            </div>
          `).join("")}
        </div>
      </div>
    </div>
  `;
}

function renderAvantaje() {
  const el = document.getElementById("avantaje-content");
  if (!el) return;
  const d = DATA.avantaje;
  el.innerHTML = `
    <div class="alt-section reveal">
      <div class="alt-text">
        <div class="section-label">${d.label}</div>
        <h2 class="section-title">${d.title}</h2>
        ${d.paragraphs.map(p => `<p>${p}</p>`).join("")}
        <ul>
          ${d.list.map(item => `
            <li>
              <span class="li-dot green">✓</span>
              ${item}
            </li>
          `).join("")}
        </ul>
      </div>
      ${imgOrPh(d.image, d.imageAlt, d.imagePlaceholder, d.imageTint)}
    </div>
  `;
}

function renderRiscuri() {
  const el = document.getElementById("riscuri-content");
  if (!el) return;
  const d = DATA.riscuri;
  el.innerHTML = `
    <div class="alt-section reverse reveal">
      ${imgOrPh(d.image, d.imageAlt, d.imagePlaceholder, d.imageTint)}
      <div class="alt-text">
        <div class="section-label">${d.label}</div>
        <h2 class="section-title">${d.title}</h2>
        ${d.paragraphs.map(p => `<p>${p}</p>`).join("")}
        <ul>
          ${d.list.map(item => `
            <li>
              <span class="li-dot red">!</span>
              ${item}
            </li>
          `).join("")}
        </ul>
      </div>
    </div>
  `;
}

function renderSiguranta() {
  const el = document.getElementById("siguranta-content");
  if (!el) return;
  const d = DATA.siguranta;
  el.innerHTML = `
    <div class="alt-section reveal">
      <div class="alt-text">
        <div class="section-label">${d.label}</div>
        <h2 class="section-title">${d.title}</h2>
        ${d.paragraphs.map(p => `<p>${p}</p>`).join("")}
        <ul>
          ${d.list.map((item, i) => `
            <li>
              <span class="li-dot blue">${i + 1}</span>
              ${item}
            </li>
          `).join("")}
        </ul>
      </div>
      ${imgOrPh(d.image, d.imageAlt, d.imagePlaceholder, d.imageTint)}
    </div>
  `;
}

function renderComparatie() {
  const el = document.getElementById("compare-grid");
  if (!el) return;
  const { bine, rau } = DATA.comparatie;
  const makeCol = (col, type) => `
    <div class="compare-col ${type}">
      <div class="compare-header">
        ${type === "good" ? "✓" : "✗"} ${col.header}
      </div>
      <div class="compare-items">
        ${col.items.map(item => `
          <div class="compare-item">
            <span class="ci-icon">${type === "good" ? "✓" : "✗"}</span>
            <span>${item}</span>
          </div>
        `).join("")}
      </div>
    </div>
  `;
  el.innerHTML = makeCol(bine, "good") + makeCol(rau, "bad");
}

function renderFAQ() {
  const el = document.getElementById("faq-list");
  if (!el) return;
  el.innerHTML = DATA.faq.map((item, i) => `
    <div class="faq-item reveal" data-index="${i}">
      <div class="faq-q" onclick="toggleFAQ(${i})">
        <span>${item.q}</span>
        <span class="faq-icon">+</span>
      </div>
      <div class="faq-a" id="faq-a-${i}">
        <div class="faq-a-inner">${item.a}</div>
      </div>
    </div>
  `).join("");
}

function toggleFAQ(index) {
  const item = document.querySelector(`.faq-item[data-index="${index}"]`);
  const answer = document.getElementById(`faq-a-${index}`);
  const isOpen = item.classList.contains("open");
  document.querySelectorAll(".faq-item.open").forEach(el => {
    el.classList.remove("open");
    el.querySelector(".faq-a").style.maxHeight = "0";
  });
  if (!isOpen) {
    item.classList.add("open");
    answer.style.maxHeight = answer.scrollHeight + "px";
  }
}

function renderFooter() {
  const el = document.getElementById("footer-content");
  if (!el) return;
  const { desc, cols } = DATA.footer;
  el.innerHTML = `
    <div class="footer-inner">
      <div class="footer-top">
        <div class="footer-brand">
          <div class="footer-logo">
            <img src="images/logo-icon.svg" alt="" onerror="this.style.display='none'" />
            AI<span>Prietenos</span>
          </div>
          <p>${desc}</p>
        </div>
        ${cols.map(col => `
          <div class="footer-col">
            <h4>${col.title}</h4>
            <ul>${col.links.map(l => `<li><a href="${l.href}">${l.label}</a></li>`).join("")}</ul>
          </div>
        `).join("")}
      </div>
      <div class="footer-bottom">
        <p>© 2025 AIPreitenos — Proiect educativ non-comercial</p>
      </div>
    </div>
  `;
}

let quizState = { current: 0, score: 0, answered: false };

function startQuizPage() {
  const card = document.getElementById("quiz-card");
  if (!card) return;
  showQuestion();
}

function showQuestion() {
  const card = document.getElementById("quiz-card");
  if (!card) return;
  const q = DATA.quiz[quizState.current];
  const pct = (quizState.current / DATA.quiz.length) * 100;
  card.innerHTML = `
    <div class="quiz-progress">
      <div class="quiz-bar-track"><div class="quiz-bar-fill" style="width:${pct}%"></div></div>
      <span class="quiz-counter">${quizState.current + 1} / ${DATA.quiz.length}</span>
    </div>
    <div class="quiz-question">${q.q}</div>
    <div class="quiz-options">
      ${q.opts.map((opt, i) => `<button class="quiz-opt" onclick="answerQuiz(${i})">${opt}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quiz-fb"></div>
    <button class="quiz-next" id="quiz-next" onclick="nextQuestion()">
      ${quizState.current < DATA.quiz.length - 1 ? "Următoarea întrebare →" : "Vezi rezultatul"}
    </button>
  `;
}

function answerQuiz(idx) {
  if (quizState.answered) return;
  quizState.answered = true;
  const q = DATA.quiz[quizState.current];
  const opts = document.querySelectorAll(".quiz-opt");
  const fb = document.getElementById("quiz-fb");
  const nextBtn = document.getElementById("quiz-next");
  opts.forEach(btn => btn.disabled = true);
  opts[q.correct].classList.add("correct");
  if (idx === q.correct) {
    quizState.score++;
    fb.className = "quiz-feedback correct";
    fb.textContent = "Corect! " + q.feedback;
  } else {
    opts[idx].classList.add("wrong");
    fb.className = "quiz-feedback wrong";
    fb.textContent = "Gresit. " + q.feedback;
  }
  nextBtn.classList.add("show");
}

function nextQuestion() {
  quizState.current++;
  quizState.answered = false;
  if (quizState.current >= DATA.quiz.length) {
    showQuizResult();
  } else {
    showQuestion();
  }
}

function showQuizResult() {
  const card = document.getElementById("quiz-card");
  const score = quizState.score;
  const total = DATA.quiz.length;
  const msg = score === total
    ? "Perfect! Ești un expert în AI responsabil!"
    : score >= total / 2
    ? "Bine! Mai ai câte ceva de învățat."
    : "Nu-i nimic! Citește din nou ghidul și încearcă din nou.";
  card.innerHTML = `
    <div class="quiz-result">
      <div class="result-icon">
        <img src="images/icon-trophy.svg" alt="" onerror="this.style.display='none'" />
        <div class="ri-ph"></div>
      </div>
      <h3>${score} din ${total} corecte</h3>
      <p>${msg}</p>
      <button class="quiz-restart" onclick="restartQuiz()">Încearcă din nou</button>
      <a href="index.html" class="quiz-back-link">Înapoi la ghid</a>
    </div>
  `;
}

function restartQuiz() {
  quizState = { current: 0, score: 0, answered: false };
  showQuestion();
}

function initScrollReveal() {
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) entry.target.classList.add("visible");
    });
  }, { threshold: 0.1 });
  document.querySelectorAll(".reveal").forEach(el => observer.observe(el));
}

function initNavbar() {
  window.addEventListener("scroll", () => {
    document.getElementById("navbar").classList.toggle("scrolled", window.scrollY > 50);
  });
}

const isIndexPage = document.getElementById("ce-este-content") !== null;

document.addEventListener("DOMContentLoaded", () => {
  renderFloatingShapes();
  renderFooter();
  initNavbar();

  if (isIndexPage) {
    renderGallery();
    renderCeEste();
    renderDomenii();
    renderAvantaje();
    renderRiscuri();
    renderSiguranta();
    renderComparatie();
    renderFAQ();
  }

  setTimeout(initScrollReveal, 100);
});